    ...
    if (res.ok) {
      ...
      if (data.trip_results && data.trip_results.length > 0) {
        ...
        if (intervalId) {
          ...
          localStorage.removeItem("lobbyRefreshInterval");
        }
...
async function generateRecommendations() {
  ...
  try {
    ...
    if (shouldUseGroup) {
      ...
      // FIX: If data is an array, use it directly. If it's an object with trip_results, use that.
      const resultsToRender = Array.isArray(data) ? data : (data.trip_results || []);
      ...
      // FIX: Ensure we are passing an array
      const resultsToRender = Array.isArray(data) ? data : (data.trip_results || []);
      renderResults(resultsToRender);
    }
  } catch (err) {
...
function renderResults(results, errorMsg) {
  console.log("=== renderResults called ===", {
    ...
    persistedResults: persistedResults ? persistedResults.length : 0
  });
  
  const container = document.getElementById("resultsContainer");
  if (!container) {
    console.error("resultsContainer not found in DOM!");
    return;
  }
  
  // REMOVED: Lock check - let's see what's happening
  // if (resultsLocked && persistedResults && persistedResults.length > 0) {
  //   console.warn("renderResults skipped because results are locked");
  //   return;
  // }
  ...
  // If we already have good results rendered and a later call comes with empty/undefined, keep the old ones.
  if ((!results || results.length === 0) && persistedResults && persistedResults.length > 0) {
    console.warn("Ignoring empty results; keeping last rendered set");
  ...
  // Save last good results to prevent flicker from later empty calls and lock
  persistedResults = results;
  resultsLocked = true;

  console.log("Building result grid with", results.length, "places");
  ...
  console.log("Created grid element:", grid);
  ...
  results.slice(0, 6).forEach((place, index) => {
    ...
    card.innerHTML = `
      <div class="result-image" style="width: 100%; height: 180px; overflow: hidden; border-radius: 8px; margin-bottom: 16px; background: #e2e8f0; position: relative;">
    ...
    console.log(`Added card ${index + 1}/${results.length} for ${place.place}`);
  });

  console.log("Grid has", grid.children.length, "cards. Now appending to container...");

  // Clear loading message and show results immediately
  if (!container) {
    console.error("Container is null before append!");
    return;
  }
  ...
  console.log("Grid appended. Container now has", container.children.length, "children");
  console.log("Grid visibility:", window.getComputedStyle(grid).display, window.getComputedStyle(grid).opacity);
  ...
  if (totalImages === 0) {
    // All images already present, no progress needed
    console.log("All images cached, skipping async fetch");
    return;
  }

  // Show progress indicator while images load (AFTER grid is already visible)
...
async function runRecommendationFlow(event) {
